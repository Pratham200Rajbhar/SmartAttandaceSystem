              ...
              <div className="space-y-1">
                {adminLinks.map((link) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`transition-colors duration-200 ${isActive ? "text-slate-300" : "text-slate-400 group-hover:text-slate-200"}`}>
                        {link.icon}
                      </span>
                      <span className="font-semibold text-sm tracking-wide">{link.label}</span>
                      {isActive && (
                        <span className="absolute right-4 w-1.5 h-1.5 rounded-full bg-indigo-400 shadow-[0_0_8px_#818cf8]" />
                      )}
                    </Link>
                  );
                })}
                      ...
                      return (
                        <Link
                          ...
                          className={`sidebar-link py-2 relative group ${isActive
                              ? "sidebar-link-active text-slate-300 bg-white/5/5 font-semibold"
                              : "text-slate-400 hover:text-slate-200"
                            }`}
                        >
                          <span className={`transition-colors duration-200 ${isActive ? "text-slate-300" : "text-slate-500 group-hover:text-slate-300"}`}>
                            {link.icon}
                          </span>
                          <span className="text-[13px] font-medium tracking-wide">{link.label}</span>
                          {isActive && (
                            <span className="absolute right-4 w-1 h-1 rounded-full bg-indigo-400 shadow-[0_0_8px_#818cf8]" />
                          )}
                        </Link>
                      );
                    })}
                  ...
                  return (
                    ...
                    >
                      ...
                      {isReviewLink && flaggedCount > 0 && (
                        <span className="ml-auto flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full bg-white/5 text-white text-[10px] font-bold shadow-[0_4px_12px_rgba(0,0,0,0.5)]">
