...
class TeacherService:
    """Service layer for all teacher-facing operations.

    Handles class management, geofencing, session attendance rosters,
    manual overrides, bulk marking, and export data preparation.
    """
    ...
    async def get_teacher_by_user_id(self, user_id: str):
        """Fetch the teacher profile for the authenticated user.
    ...
    def _resolve_full_name(self, first: Optional[str], last: Optional[str]) -> str:
        """Safely combine first and last name with null coalescing."""
    ...
    async def get_classes_by_teacher_user_id(
        ...
        """Return all classes assigned to the teacher, with geofence and subject resolved."""
    ...
    async def get_session_attendance_roster(
        ...
        Builds a combined list of all enrolled students, merging in any
        existing attendance records (students without records are shown as Absent).
        """
        teacher = await self.get_teacher_by_user_id(user_id)
    ...
    async def get_absent_students(
        ...
        """Return all enrolled students who have not been marked for a session.

        Used by the Manual Entry page to show only the students still needing action.
        """
        teacher = await self.get_teacher_by_user_id(user_id)
    ...
    async def bulk_mark_attendance(
        ...
        """Mark or update attendance for multiple students in one call.

        Uses upsert so it is safe to call for students who already have a record
        (e.g. "Mark All Absent → Present" scenario). Returns count of records processed.
        """
        teacher = await self.get_teacher_by_user_id(user_id)
    ...
    async def export_class_attendance(
        ...
        """Build the flat export data for a class attendance CSV.

        Applies optional date range filtering on session start time.
        Returns a list of dicts that can be directly fed to Papa.unparse on the frontend.
        """
        teacher = await self.get_teacher_by_user_id(user_id)
    ...
    async def manual_override_attendance(
        ...
        """Apply a single-student manual override for a session.

        Verifies that the teacher owns the class and the student is enrolled
        before upserting the attendance record.
        """
        teacher = await self.get_teacher_by_user_id(user_id)
    ...
    async def get_teacher_sessions(self, user_id: str) -> List[SessionWithClassResponse]:
        """Return all sessions for the teacher's classes with resolved subject names."""
        teacher = await self.get_teacher_by_user_id(user_id)
