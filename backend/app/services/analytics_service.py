"""
Analytics service - at-risk detection, predictive analytics, and early warning system.
"""
import logging
...
class AnalyticsService:
    """Advanced analytics and at-risk student detection."""
    ...
    async def detect_at_risk_students(self) -> List[Dict]:
        """
        Detect students who are at risk of failing attendance requirements.
        
        Criteria:
        1. Attendance percentage < 75%
        2. Attendance dropped by >20% in last 14 days compared to historical average
        ...
        for student in all_students:
            ...
            if len(all_records) < 5:  # Skip students with too few records
                continue
    ...
    async def _calculate_recent_attendance(self, student_id: str, days: int) -> float:
        """Calculate attendance percentage for recent period."""
    ...
    async def _calculate_historical_attendance(self, student_id: str, days: int) -> float:
        """Calculate attendance percentage for historical period."""
        end_date = datetime.now(timezone.utc) - timedelta(days=14)  # Exclude recent 14 days
        start_date = end_date - timedelta(days=days)
    ...
    def _calculate_risk_level(self, factor_count: int, overall_pct: float) -> str:
        """Determine risk level based on factors and percentage."""
